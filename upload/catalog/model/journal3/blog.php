...
class ModelJournal3Blog extends Model {
	...
	public function getPosts($data = array()) {
		...
		$sql .= "
            ...
            LEFT JOIN
            	`{$this->dbPrefix('user')}` a ON p.author_id = a.user_id
            WHERE pd.language_id = '{$this->dbEscapeInt($this->config->get('config_language_id'))}' AND p2s.store_id = '{$this->dbEscapeInt($this->config->get('config_store_id'))}'
        ";
		...
		if ($limit) {
			$sql .= " LIMIT " . (int)Arr::get($data, 'start', 0) . "," . (int)$limit;
		}
	...
	public function getRelatedPosts($product_id, $limit = 5) {
		...
		$sql = "
            ...
            WHERE pd.language_id = '{$this->dbEscapeInt($this->config->get('config_language_id'))}' AND p2p.product_id = {$product_id} AND p.status = 1
	...
	public function getRelatedProducts($post_id, $limit = 5) {
		...
		if ($limit) {
			$sql .= "
				LIMIT 0, {$this->dbEscapeInt($limit)}
			";
		}
	...
	public function createComment($data) {
		...
		$name = $this->db->escape(Arr::get($data, 'name', ''));
		$email = $this->db->escape(Arr::get($data, 'email', ''));
		$website = $this->db->escape(Arr::get($data, 'website', ''));
