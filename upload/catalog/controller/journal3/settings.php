	...
	public function index() {
		...
		if ($this->_cache === false) {
			...
			if ($data = Arr::get($cache, 'php.headerDesktop')) {
				...
				$settings = $this->model_journal3_module->get($module_id, $module_type);
			...
			$cache['css'] .= strip_tags($this->load->view('journal3/css', array('data' => $cache['php'])));

//			$files = glob($this->journal3->minifier->getAssetsPath() . 'style.*');
//
//			foreach ($files as $file) {
//				if (is_file($file)) {
//					@unlink($file);
...
//			}
//
//			file_put_contents($this->journal3->minifier->getAssetsPath() . $cache['php']['css_file'], $cache['css']);
//
//			unset($cache['css']);
