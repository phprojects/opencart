...
class ControllerJournal3Filter extends ModuleController {
	...
	public function index($args) {
		...
		if ($this->_cache === false) {
			...
			$this->module_data = $this->_cache['module_data'];
		}
	...
	protected function beforeRender() {
		...
		if ($price_range['min'] !== $price_range['max']) {
			$item = Arr::get($this->settings['items'], 'p');
		...
		// categories
		if ($categories = $this->model_journal3_filter->getCategories()) {
			$item = Arr::get($this->settings['items'], 'c');
		...
		// manufacturers
		if ($manufacturers = $this->model_journal3_filter->getManufacturers()) {
			$item = Arr::get($this->settings['items'], 'm');
		...
		// attributes
		foreach ($this->model_journal3_filter->getAttributes() as $attributes) {
		...
		// options
		foreach ($this->model_journal3_filter->getOptions() as $options) {
		...
		// filters
		foreach ($this->model_journal3_filter->getFilters() as $filters) {
		...
		// tags
		if ($tags = $this->model_journal3_filter->getTags()) {
			$item = Arr::get($this->settings['items'], 't');
