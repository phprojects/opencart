...
class Document {
	...
	public function getCss() {
		$css = array_reduce($this->css, function ($carry, $css) {
			return $carry . PHP_EOL . $css;
		}, '');
...
//		$this->addStyle(Journal3::getInstance()->minifier->getAssetsUrl() . Journal3::getInstance()->settings->get('css_file'));
	...
	public function staticUrl($url, $cache = true) {
		...
		if (Journal3::getInstance()->settings->get('performanceQueryStrings')) {
			...
			return $url . '?v=' . (defined('JOURNAL3_BUILD') ? JOURNAL3_BUILD : JOURNAL3_VERSION);
		}
