<?php 
class ControllerExtensionModulemassppriceupd extends Controller
{
    private $error = array();
    ...
    public function index()
    {
        ...
        $this->load->language('extension/module/massppriceupd');

        $this->document->addStyle('view/template/extension/module/massppriceupd.css');
        ...
        $data['masstxt_p_updates_price'] = $this->language->get('masstxt_p_updates_price');
        $data['masstxt_p_updates_value'] = $this->language->get('masstxt_p_updates_value');
        $data['masstxt_no_update_price_help'] = $this->language->get('masstxt_no_update_price_help');
        $data['masstxt_p_updates_tax_class'] = $this->language->get('masstxt_p_updates_tax_class');
        $data['masstxt_no_update_tax_class_help'] = $this->language->get('masstxt_no_update_tax_class_help');
        $data['masstxt_p_updates_date_available'] = $this->language->get('masstxt_p_updates_date_available');
        $data['masstxt_no_update_date_a_help'] = $this->language->get('masstxt_no_update_date_a_help');
        $data['masstxt_p_updates_status'] = $this->language->get('masstxt_p_updates_status');
        $data['masstxt_no_update_status_help'] = $this->language->get('masstxt_no_update_status_help');

        $data['masstxt_upd_price_addition_help'] = $this->language->get('masstxt_upd_price_addition_help');
        $data['masstxt_upd_price_subtraction_help'] = $this->language->get('masstxt_upd_price_subtraction_help');
        $data['masstxt_upd_price_multiplication_help'] = $this->language->get('masstxt_upd_price_multiplication_help');
        $data['masstxt_upd_price_replacement_help'] = $this->language->get('masstxt_upd_price_replacement_help');
        ...
        if (isset($this->request->post['price_val_upd'])) {
            $data['price_val_upd'] = $this->request->post['price_val_upd'];
        } else {
            $data['price_val_upd'] = '';
        }

        if (isset($this->request->post['tax_class_upd'])) {
            $data['tax_class_upd'] = $this->request->post['tax_class_upd'];
        } else {
            $data['tax_class_upd'] = 'no-update';
        }

        if (isset($this->request->post['date_av_upd'])) {
            $data['date_av_upd'] = $this->request->post['date_av_upd'];
        } else {
            $data['date_av_upd'] = '';
        }

        if (isset($this->request->post['status_upd'])) {
            $data['status_upd'] = $this->request->post['status_upd'];
        } else {
            $data['status_upd'] = 'no-update';
        }
        ...
        if (isset($this->request->post['mass_update'])) { /// update button
            ...
            if ($this->user->hasPermission('modify', 'extension/module/massppriceupd')) { /// modify permision

                if (isset($this->request->post['selected'])) { /// avem produse selectate

                    if ($this->request->post['price_val_upd'] != "" or $this->request->post['tax_class_upd'] != "no-update" or $this->request->post['date_av_upd'] != "" or $this->request->post['status_upd'] != "no-update") { /// avem update
                        ...
                        foreach ($this->request->post['selected'] as $product_id) { /// scanare produse
                            ...
                            if ($this->request->post['price_val_upd'] != "") { /// avem price update
                                switch ($this->request->post['upd_mode']) {
                                    case "ad":
                                        if ($plus_set == "") {
                                            $prfx = " SET ";
                                        } else {
                                        ...
                                        $plus_set .= $prfx . "p.price = (p.price + '" . (float)$this->request->post['price_val_upd'] . "')";
                                        break;
                                    case "su":
                                        if ($plus_set == "") {
                                            $prfx = " SET ";
                                    ...
                                    case "re":
                                        ...
                                        $plus_set .= $prfx . "p.price = '" . (float)$this->request->post['price_val_upd'] . "'";
                                        break;
                                }
                            }

                            if ($this->request->post['tax_class_upd'] != "no-update") { /// avem tax class update
                                if ($plus_set == "") {
                                    $prfx = " SET ";
                                } else {
                                    $prfx = ", ";
                                }
                                $plus_set .= $prfx . "p.tax_class_id = '" . (int)$this->request->post['tax_class_upd'] . "'";
                            }

                            if ($this->request->post['date_av_upd'] != "") { /// avem Date Available update
                                if ($plus_set == "") {
                                    $prfx = " SET ";
                                } else {
                                    $prfx = ", ";
                                }
                                $plus_set .= $prfx . "p.date_available = '" . $this->db->escape($this->request->post['date_av_upd']) . "'";
                            }

                            if ($this->request->post['status_upd'] != "no-update") { /// avem Status update
                                if ($plus_set == "") {
                                    $prfx = " SET ";
                                } else {
                                    $prfx = ", ";
                                }
                                $plus_set .= $prfx . "p.status = '" . (int)$this->request->post['status_upd'] . "'";
                            }

                            $this->db->query("UPDATE " . DB_PREFIX . "product p" . $plus_set . ", date_modified = NOW() WHERE product_id = '" . (int)$product_id . "'");
                        } /// end scanare produse
        ...
        if (isset($this->request->post['lista_prod']) or isset($this->request->post['mass_update'])) { /// data filters
            ...
            if ($this->request->post['filter_attr_val'] != "") { // attribute value (text)
                if ($this->request->post['filter_attr'] == "any") {
                    $plus_join .= " LEFT JOIN " . DB_PREFIX . "product_attribute pattr ON (p.product_id = pattr.product_id)";
                }
                if ($plus_where == "") {
                    $prfx = " WHERE ";
...
" . $plus_join . $plus_where;
        ...
        if ($data['curent_pag'] > $data['total_pag']) {
            $data['curent_pag'] = $data['total_pag'];
        }
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/module/massppriceupd', 'user_token=' . $this->session->data['user_token'], true)
        );
        ...
        $data['action'] = $this->url->link('extension/module/massppriceupd', 'user_token=' . $this->session->data['user_token'], true);

        $data['header'] = $this->load->controller('common/header');
        $data['column_left'] = $this->load->controller('common/column_left');
        $data['footer'] = $this->load->controller('common/footer');

        $this->response->setOutput($this->load->view('extension/module/massppriceupd', $data));
    }
}
